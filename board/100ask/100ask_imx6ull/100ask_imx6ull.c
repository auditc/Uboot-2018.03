...
int is_fastboot_key_pressing(void)
{
	...
	imx_iomux_v3_setup_multiple_pads(user_key_pads,
		ARRAY_SIZE(user_key_pads));

	gpio_request(GPIO_USER_KEY, "user key");
	gpio_direction_input(GPIO_USER_KEY);
	if (gpio_get_value(GPIO_USER_KEY) == 0) { /* VOL_DN key is low assert */
		button_pressed = 1;
